...
-(id)initWithParentView:(UIView*)parentView withMode:(FlatDatePickerMode)mode withDataSource:(NSMutableArray *)objects{
    _parentView = parentView;
    if ([self initWithFrame:CGRectMake(0, screenHeight - UpHeight - kFlatDatePickerHeight, _parentView.frame.size.width, kFlatDatePickerHeight)])
    {
        _datePickerMode = mode;
        [_parentView addSubview:self];
        [self setupControl:objects];
    }
    return self;
}

-(void)setupControl:(NSMutableArray *)objects{
    ...
    if(_datePickerMode == CustomFlatPickerMode){
        [self buildHeader];
        _years = objects;
        [self buildSelectorYearsOffsetX:0.0
                               andWidth:_parentView.frame.size.width];
        [self setScrollView:_scollViewDays atIndex:0 animated:YES];
    }
}
...
- (void)actionButtonValid {
    ...
    if(_datePickerMode == CustomFlatPickerMode){
        if (self.delegate != nil && [self.delegate respondsToSelector:@selector(flatDatePicker:didValid:selectObject:)]) {
            int index = [self getIndexForScrollViewPosition:_scollViewYears];
            [self.delegate flatDatePicker:self didValid:_buttonValid selectObject:[_years objectAtIndex:index]];
